...
export function CartSidebar() {
  ...
  const handleWhatsAppOrder = () => {
    const text = `Hi Gorakhpur Diaper House!\n\nI want to order:\n${cartItems.map(item => `- ${item.product.name} (x${item.qty}) = ₹${item.product.price * item.qty}`).join('\n')}\n\nTotal: ₹${subtotal}\n\nPlease let me know the delivery details.`;
      ...
      <SheetContent className="w-full sm:max-w-md bg-[#FFFAF8] flex flex-col p-0 border-l-[#F4A0B0]/30">
        ...
        {cartItems.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
            ...
            <Button onClick={() => setIsCartOpen(false)} className="bg-[#E8547A] hover:bg-[#D43D63] text-white rounded-xl h-11 px-8 font-bold">
              Start Shopping
            </Button>
          </div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
              {cartItems.map(item => (
                <div key={item.productId} className="flex gap-4 bg-white p-3 rounded-2xl border border-border shadow-sm">
                  <div 
                  ...
                  <div className="flex-1 flex flex-col">
                    <h4 className="font-bold text-sm leading-tight text-foreground mb-1 line-clamp-2">
                      {item.product.name}
                    </h4>
